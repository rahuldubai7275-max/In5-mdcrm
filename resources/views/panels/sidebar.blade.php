        ...
        <ul class="nav navbar-nav flex-row">
            <li class="nav-item mr-auto"><a class="navbar-brand" target="_blank" href="/">
                    @php
                        ...
                        $BrandLogo='/storage/'.$company->logo;//LOGO;
                        $hr_admin=\App\Models\Admin::where('type',6)->pluck('id')->toArray();

                        $hr_access_setting=\App\Models\Setting::where('company_id',$admin->company_id)->where('title','hr_access')->first();
                        $hr_access=[];
                        if($hr_access_setting)
                            $hr_access=\App\Models\SettingAdmin::where('setting_id',$hr_access_setting->id)->pluck('admin_id')->toArray();

                        $survey_access_setting=\App\Models\Setting::where('company_id',$admin->company_id)->where('title','survey_access')->first();
                        $survey_access=[];
                        if($survey_access_setting)
                            $survey_access=\App\Models\SettingAdmin::where('setting_id',$survey_access_setting->id)->pluck('admin_id')->toArray();

                        $task_access=\App\Models\Setting::where('company_id',$admin->company_id)->where('title','all_task')->first();
                        $task_access_admin=[];
                        if($task_access)
                            $task_access_admin=\App\Models\SettingAdmin::where('setting_id',$task_access->id)->pluck('admin_id')->toArray();

                        $request_approver_setting=\App\Models\Setting::where('company_id',$admin->company_id)->where('title','request_approver')->first();
                        $request_approver_admin_id=[];
                        if($request_approver_setting)
                            $request_approver_admin_id=\App\Models\SettingAdmin::where('setting_id',$request_approver_setting->id)->pluck('admin_id')->toArray();

                        $request_main_setting=\App\Models\Setting::where('company_id',$admin->company_id)->where('title','request_main_approver')->first();
                        $request_main_admin_id=[];
                        if($request_main_setting)
                            $request_main_admin_id=\App\Models\SettingAdmin::where('setting_id',$request_main_setting->id)->pluck('admin_id')->toArray();
                    ...
                    <h2 class="brand-text mb-0">{{$company->brand}}</h2>
                </a></li>
            ...
            @if(isset($menuData[$MenuIndex]))
                @foreach($menuData[$MenuIndex]->menu as $menu)
                    ...
                    @else
                        ...
                        @php
                            ...
                            if($menu->name!='User Guide'){
                                if(in_array($admin->id, $hr_admin) &&
                                   ($menu->name!='Dashboard' && $menu->name!='Calendar' && $menu->name!='Report' && $menu->name!='Users' && $menu->name!='Survey Report' && $menu->name!='Requests'))
                                   continue;

                                //if(($admin->type>2 && $menu->name!='Property') && ($admin->type>2 && $menu->name!='Contacts'))
                                if(($admin->type>2 && $menu->name=='Admin')
                                   || ($admin->type>2 && $menu->name=='Company'))
                                  continue;
                                ...
                                if($admin->type==8 && $menu->name!='Leave Requests')
                                    continue;
                                ...
                                if(($admin->super!=1 || $company->package==1) && $menu->name=='Control Panel')
                                    continue;
                                ...
                                if( (!in_array($admin->id, $survey_access) || $company->package==1) && $menu->name=='Survey Report')
                                    continue;
                                ...
                                if(($admin->super==1 || $company->package==1) && $menu->name=='Survey Report')
                                    continue;
                                if(($admin->super!=1 || $company->package==1) && $menu->name=='Survey')
                                    continue;

                                if($admin->super!=1 && $menu->name=='Task')
                                    continue;

                                if($admin->type>2 && $menu->name=='Users')
                                    continue;
                        ...
                        @endif
                        <li class="nav-item {{ ($menu->name!='Dashboard' && $menu->name!='New Projects' && $menu->name!='Calendar' && $menu->name!='Report' && $menu->name!='Control Panel') ? 'd-none d-sm-block' : '' }} {{ (request()->is(substr($menu->url,1))) ? 'active' : '' }} {{ $custom_classes }}">
