        ...
        <div class="col-md-4 col-sm-12">
            <div class="card">
                ...
                <div class="card-content collapse show">
                    <div class="card-body">
                        <form id="record-form" method="post" action="{{ route('job-title.add') }}" class="form error" novalidate>
                            @csrf
                            <div class="form-body">
                                <div class="row">
                                    <div class="col-12">
                                        <div class="form-label-group">
                                            <input type="text" id="name" class="form-control" placeholder="Job Title" name="name" required="required">
                                            <label for="Name">Job Title</label>
                                        </div>
                                    </div>
                                    <input type="hidden" name="possession_or_job" value="{{$possession_or_job}}">
                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table zero-configuration table-striped">
                                ...
                                <tbody>
                                @php
                                    $JobTitle=\App\Models\JobTitle::where('possession_or_job',$possession_or_job)->get();
                                @endphp
                                @foreach($JobTitle as $row)
                                    <tr>
                                        <td>{{ $row->name }}</td>
                                        <td>
                                            <div data-id="{{ $row->id }}" data-model="{{ route('job-title.delete') }}" data-edit="{{ route('job-title.edit') }}"
