...
//                $where .= ' AND leads.status!=3';
    ...
    public function pfLead(){
        ...
        post('https://atlas.propertyfinder.com/v1/auth/token');
        $token_response= json_decode($token_response);
        $response = Http::withToken($token_response->accessToken)->get('https://atlas.propertyfinder.com/v1/leads?page=10&createdAtFrom=2026-01-01T00:00:00Z&createdAtTo=2026-01-31T23:59:59Z');//?createdAtFrom='.date('Y-m-d').'T00:00:00Z'
        //$response = Http::withToken($token_response->accessToken)->get('https://atlas.propertyfinder.com/v1/leads');//?createdAtFrom='.date('Y-m-d').'T00:00:00Z'
    ...
    public function bayutLead(){
        ...
        $now = strtotime(date('Y-m-d H:i:s') . ' - 10 minute');
        $date_time = '2026-01-01 00:00:00';date('Y-m-d H:i:s', $now);
        ...
        if ($response) {
            if ($type == 'leads') {
                ...
                foreach ($response as $row) {
                    ...
                    $mobile_number = str_replace('-', '', filter_var($row->client_phone, FILTER_SANITIZE_NUMBER_INT));
                    $lead_check = Lead::where('company_id', $company->id)->where('mobile_number', $mobile_number)->where('created_at', $row->date_time)->first();
            ...
            if ($type == 'call_logs') {
                foreach ($response as $row) {
                    ...
                    $mobile_number=str_replace('-', '', filter_var($row->caller_number, FILTER_SANITIZE_NUMBER_INT));
                    $lead_check = Lead::where('company_id', $company->id)->where('mobile_number', $mobile_number)->where('created_at', $row->date . ' ' . $row->time)->first();
